...
pub async fn ai_parse(app: AppHandle, text: String) -> Result<Vec<AiParseResult>, String> {
    ...
    let system_prompt = r#"你是一个待办事项解析器。将用户的自然语言输入解析为结构化的待办事项数据。
...
- date: 提取日期（格式 YYYY-MM-DD），无明确日期则 null
...
- block_name: 判断属于哪个区块，用户没指定时默认"待办"
- reason: 简要说明为什么这样解析
...
输入："周内注册公司\n明天买牛奶"
输出：[{"content":"注册公司","date":"2026-06-30","item_type":"todo","priority":"important_not_urgent","block_name":"待办","reason":"周内需要完成"},{"content":"买牛奶","date":"2026-06-30","item_type":"todo","priority":"neither","block_name":"购物清单","reason":"日常购物"}]"#;
